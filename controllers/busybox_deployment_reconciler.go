...
func (r *BusyBoxDeploymentReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	l := log.FromContext(ctx).WithValues("reconciler", "busybox")
	...
	l.Info("Resolving Deployment Image", "image", image.BusyBox())
	new := appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "busybox-" + instanceKey.Name,
		...
		Spec: appsv1.DeploymentSpec{
			Replicas: instance.BusyBoxReplicas(),
